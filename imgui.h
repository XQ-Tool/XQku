...
struct ImDrawData;                  // All draw command lists required to render the frame + pos/size coordinates to use for the 世界投影 matrix.
...
namespace ImGui
{
    ...
    IMGUI_API ImVec2        CalcTextSize(ImFont *font, const char *text, float font_size);
...
struct ImDrawData
{
    ...
    ImVec2              DisplayPos;         // Top-left position of the viewport to render (== top-left of the orthogonal 世界投影 matrix to use) (== GetMainViewport()->Pos for the main viewport, == (0.0) in most single-viewport applications)
